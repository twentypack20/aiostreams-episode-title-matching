...
export async function validateConfig(
  ...
  const needTmdb =
    ...
    config.episodeTitleMatching?.enabled ||
    config.digitalReleaseFilter?.enabled ||
    config.bitrate?.useMetadataRuntime;

  if (needTmdb && !tmdbAuth) {
    throw new Error(
      'A TMDB API key or access token is required for the following features: title matching, year matching, episode title matching, and digital release filter'
    );
  }
